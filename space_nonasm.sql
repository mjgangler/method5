...
select * from dba_scheduler_job_run_details where job_name = 'GATHER_NONASM_JOB' order by log_date desc;
select * from dba_scheduler_jobs where job_name = 'GATHER_NONASM_JOB';

select * from dba_scheduler_job_run_details where job_name = 'EMAIL_NONASM_FORECAST_JOB' order by log_date desc;
select * from dba_scheduler_jobs where job_name = 'EMAIL_NONASM_FORECAST_JOB';

--Check database results.  It should work about 99% of the time.
select * from nonasm_diskgroup_fcst_meta order by date_started desc;
select * from nonasm_diskgroup_fcst_err order by date_error desc;
    ...
    m5_proc(
        ...
        select trunc(sysdate) the_date,
									nvl(b.tablespace_name,
                                     ...
                                     ((kbytes_alloc-nvl(kbytes_free,0))/kbytes_alloc) used_percent,
                                     data_files 
...
begin
    dbms_scheduler.run_job('GATHER_NONASM_JOB');
end;
/
...
alter table nonasm_diskgroup_fcst add constraint nonasm_diskgroup_fcst_uq unique (database_name, the_date, tablespace);
...
create table nonasm_forecast_config
(
...
insert into nonasm_forecast_config(config_name, config_string) values ('MAIL_HOST', 'localhost');
insert into nonasm_forecast_config(config_name, config_number) values ('MAIL_PORT', 25);
insert into nonasm_forecast_config(config_name, config_string) values ('SENDER_AND_FROM_AND_REPLY_TO', 'method5_s24dba@secure-24.com');
insert into nonasm_forecast_config(config_name, config_string) values ('RECIPIENT_AND_TO', 'oracleteam@secure-24.com');


--#3c: Create procedure to email results.
create or replace procedure email_nonasm_forecast authid current_user is
/*
    Purpose: Send an email of the Top N forecasted ASM "Tablespace"s.
...
The table below shows the 10 "Tablespace"s that are predicted as most likely to run out of space.
These predictions are based on the worst of three forecasts.
The forecasts use the last 2, 7, and 30 days of data to predict "Tablespace" growth over the next month.
...
');
    ...
    (
                                        ...
                                        (
                                            ...
                                            (
                                                ...
                                                (
                                                    ...
                                                    on nonasm_"Tablespace"_fcst.database_name = databases.database_name
                                                where total_mb <> 0
        ...
        --Write line to email.
        utl_smtp.write_data(conn, "Tablespace"s.html || chr(10));
    end loop;
...
begin
    dbms_scheduler.create_job(
        job_name => 'EMAIL_NONASM_FORECAST_JOB',
        job_type => 'PLSQL_BLOCK',
        start_date => systimestamp at time zone 'US/Eastern',
        enabled => true,
        --Make sure this is scheduled *after* the NONASM gather step above.
        repeat_interval => 'freq=daily; byhour=2; byminute=10; bysecond=0',
        job_action => 'begin email_nonasm_forecast; end;');
end;
/
...
begin
    dbms_scheduler.run_job('EMAIL_NONASM_FORECAST_JOB');
end;
/
