...
select * from dba_scheduler_job_run_details where job_name = 'GATHER_STORAGE_JOB' order by log_date desc;
select * from dba_scheduler_jobs where job_name = 'GATHER_STORAGE_JOB';

select * from dba_scheduler_job_run_details where job_name = 'EMAIL_STORAGE_FORECAST_JOB' order by log_date desc;
select * from dba_scheduler_jobs where job_name = 'EMAIL_STORAGE_FORECAST_JOB';
...
--MJGselect * from v$asm_diskgroup_fcst_err order by date_error desc;
...
create or replace procedure gather_nonasm_disk_and_disgroup authid current_user is
...
begin
    dbms_scheduler.create_job(
        job_name => 'GATHER_STORAGE_JOB',
        job_type => 'PLSQL_BLOCK',
        start_date => systimestamp at time zone 'US/Eastern',
        enabled => true,
        repeat_interval => 'freq=daily; byhour=1; byminute=50; bysecond=0',
        job_action => 'begin gather_nonasm_disk_and_disgroup; end;');
end;
/
...
begin
    dbms_scheduler.run_job('GATHER_storage_JOB');
end;
/
