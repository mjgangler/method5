--nstall Method5 on remote target.
	...
	if lower(sys_context('userenv', 'db_name')) = 'oem12dg' then
		...
		$if dbms_db_version.ver_le_11_2 $then
			if v_sec_case_sensitive_logon = 'TRUE' then
				execute immediate q'!create user method5 profile ZENOSS_PROFILE identified by values 'S:6398859AEA1DC58D653F232E9D58961D6442BC0AE6E1E36C5D3C1B68B7FD'!';
			else
				if 'E58508718E1CB0D5' is null then
					...
					execute immediate q'!create user method5 profile ZENOSS_PROFILE identified by values 'E58508718E1CB0D5'!';
				end if;
			end if;
		--Change the hash for 12c.
		$else
			if v_sec_case_sensitive_logon = 'FALSE' then
                           execute immediate q'!create user method5 profile ZENOSS_PROFILE identified by values 'E58508718E1CB0D5'!';
                        else
			execute immediate q'!create user method5 profile ZENOSS_PROFILE identified by values 'S:6398859AEA1DC58D653F232E9D58961D6442BC0AE6E1E36C5D3C1B68B7FD;T:42CE0BAAAE4FD745BF6C1BB716F5400AE4E7870E000C3D0BEF7D61F854C4D76758F0C7562174D8A1B92E4F112ECC11B50324A1C9D76431AAF63D155FDCFE31364C7FEE451E28566E38B67CFD517F70B5'!';
                        end if;
		$end
	end if;
end;
